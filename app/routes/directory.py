    ...
    try:
        ...
        for item in sorted(os.listdir(path)):
            ...
            if os.path.isdir(item_path):
                directories.append({"name": item, "url": relative_path.replace("\\", "/")})
            else:
                files.append({"name": item, "url": relative_path.replace("\\", "/")})

        return {"status": "success", "files": files, "directories": directories}

    except Exception as e:
        logger.error(f"Error listing directory: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="An error occurred while listing the directory")
